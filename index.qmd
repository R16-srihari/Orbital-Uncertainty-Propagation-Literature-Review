...
2.  **Initial Moment Characterization:** Compute the raw moments of the initial PDF. Since STT methods propagates the deviation $\delta \mathbf{x}(t)$, the initial moments are computed for the zero-mean deviation $\delta \mathbf{x}_0$ rather than the absolute state $\mathbf{x}_0$. For a Gaussian initial distribution, the moments can be computed analytically using **Isserlis' Theorem** (or Wick's formula).
