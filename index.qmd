...
@eq-stt-deviation defines the deviation of a current state from a nominal trajectory as the difference between two solution flows of the system's equations of motion, which are of the form $\dot{\mathbf{x}} = \mathbf{f}(\mathbf{x}, t)$.
...
The above deviation follows the dynamics of the system, which can be expressed as a differential equation for the deviation:
$$
\dot{\delta \mathbf{x}}(t) = \mathbf{f}(x+\delta x, t) - \mathbf{f}(x, t)
$$ {#eq-stt-eom}
...
Using similar logic, we can also derive the Taylor expansion for the equations of motion (EOM) from @eq-stt-eom for the perturbed state, which is essential for propagating the STTs through time:

$$
\dot{\delta x}_i(t) \approx \sum_{p=1}^P \frac{1}{p!} \mathbf{f}^*_{i,k_1 \dots k_p}(t, t_0) \delta x_{k_1} \dots \delta x_{k_p}
$$ {#eq-stt-eom-taylor-map}

:::{.column-margin}
$$
\mathbf{f}^*_{i,k_1 \dots k_p}(t, t_0) = \left. \frac{\partial^p f_i}{\partial x_{k_1} \dots \partial x_{k_p}} \right|_{\mathbf{x}^*(t)}
$$ 

where the superscript $*$ indicates evaluation along the nominal trajectory $\mathbf{x}^*(t)$.
:::
...
STTs are obtained by integrating a set of coupled ordinary differential equations (ODEs) obtained by substituting @eq-stt-taylor-map and @eq-stt-taylor-map-time-derivative into RHS and LHS of @eq-stt-eom-taylor-map each. (Derivation is provided in @sec-appendix-stt). For example, the second-order STT(i.e, $P=2$) evolves according to the following ODE:
...
# B: Derivation of the differential equation for the second-order State Transition Tensor (STT) propagation {#sec-appendix-stt .appendix}
...
Below is the detailed derivation as described in @park-scheeres:10.2514/1.20177.

---

## Step 1. Taylor expansion of the state (@eq-stt-taylor-map) {.appendix}
...
## Step 2. Time derivative of @eq-stt-taylor-map {.appendix}
...
which is @eq-stt-taylor-map-time-derivative. The first term is first-order (hence ignored in the second order derivation), and the second term is second-order.

---

## Step 3. Expand the dynamics {.appendix}
...
## Step 4. Substitute @eq-stt-taylor-map into @eq-stt-eom-taylor-map {.appendix}
...
The right-hand side from @eq-stt-taylor-map-time-derivative contains
