...
The fundamental premise of the STT method is that the state deviation $\delta \mathbf{x}(t)$ and its derivative $\dot{\delta \mathbf{x}(t)}$ at a future time can be expressed as functions of the initial deviation $\delta \mathbf{x}_0$ using a $P$-order Taylor series expansion about a nominal trajectory $\mathbf{x}^*(t)$.
...
We can also derive the Taylor expansion for dynamics deviation caused by the initial uncertainty from @eq-stt-eom via a similar pathway. This yields the following expression for the time derivative of the deviation:
...
We can also find $\dot{x}_i(t)$ by differentiating @eq-stt-taylor-map with respect to time, which yields:
$$
...
2. We then equate this new expression with @eq-stt-taylor-map-time-derivative, which also expresses $\dot{\delta x}_i(t)$ in terms of the initial deviations and the time derivatives of the STTs. The coefficients of the same monomials $\delta x_{0,k_1} \dots \delta x_{0,k_p}$ on both sides of the equation must be equal, leading to a set of ODEs for the STTs.
...
$$ {eq-stt-first-order-differential-equation}
...
The first piece is first-order and contributes to the first-order equation, while the second piece contributes to the second-order equation.
...
are independent variables, their coefficients must be equal. We equate the coefficients of $\delta x_{0a}\delta x_{0b}$ from both sides:
...
$$ {eq-stt-second-order-differential-equation}
