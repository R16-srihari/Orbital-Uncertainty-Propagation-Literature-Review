...
3.  **Statistical Mapping:** Use the Taylor expansion and the linearity of the expectation operator to compute the terminal mean ($\hat{\mu}$) and covariance ($\hat{\mathbf{P}}$):
    ...
    $$
    P_{ij}(t)=\sum_{p=1}^{m}\sum_{q=1}^{m}\frac1{p!q!}\Phi_{i;k_1\cdots k_p}\Phi_{j;l_1\cdots l_q}E[\tilde x_{k_1}\cdots\tilde x_{k_p}\tilde x_{l_1}\cdots\tilde x_{l_q}]-\delta \mu_i\delta \mu_j
    $$

    where $\delta \mu_i$ is the $i$-th component of the terminal mean vector and $P_{ij}$ is the $(i,j)$-th component of the terminal covariance matrix.
