...
\begin{itemize}
\item
  \(\delta \mathbf{x}(t)\): The vector of state deviations at time
  \(t\).
\item
  \(\phi(t; \mathbf{x}_0, t_0)\): The \textbf{solution flow},
  representing the mapping of the state from the initial epoch \(t_0\)
  to a future time \(t\).
\item
...
We can also derive the Taylor expansion for dynamics deviation caused by
the initial uncertainty from Equation~\ref{eq-stt-eom} via a similar
pathway. This yields the following expression for the time derivative of
the deviation:
...
Equation~\ref{eq-stt-taylor-map} with respect to time, which yields:
...
\item
  ...
  equation must be equal, leading to a set of ODEs for the STTs.
\end{enumerate}
...
\item
  ...
  \(\mathbf{x}(t)\) and the tensors \(\phi\) up to order \(P\). The
  value of \(P\) is chosen based on the nonlinearity index taken from
  the propagation of the deviation \(\delta \mathbf{x}(t)\).
\end{enumerate}
...
state
(Equation~\ref{eq-stt-taylor-map})}{1. First order Taylor expansion of the state (Equation~)}}\label{first-order-taylor-expansion-of-the-state-eq-stt-taylor-map}
...
\subsubsection{\texorpdfstring{2. Time derivative of
Equation~\ref{eq-stt-taylor-map}}{2. Time derivative of Equation~}}\label{time-derivative-of-eq-stt-taylor-map}
...
\subsubsection{\texorpdfstring{4. Substitute
Equation~\ref{eq-stt-taylor-map} into
Equation~\ref{eq-stt-eom-taylor-map}}{4. Substitute Equation~ into Equation~}}\label{substitute-eq-stt-taylor-map-into-eq-stt-eom-taylor-map}
...
\subsubsection{5. Balance coefficients}\label{balance-coefficients}
...
state
(Equation~\ref{eq-stt-taylor-map})}{1. Second order Taylor expansion of the state (Equation~)}}\label{second-order-taylor-expansion-of-the-state-eq-stt-taylor-map}
...
\subsubsection{\texorpdfstring{2. Time derivative of
Equation~\ref{eq-stt-taylor-map}}{2. Time derivative of Equation~}}\label{time-derivative-of-eq-stt-taylor-map-1}
...
\subsubsection{\texorpdfstring{3. Dynamics deviation taylor map first
order
(Equation~\ref{eq-stt-eom-taylor-map})}{3. Dynamics deviation taylor map first order (Equation~)}}\label{dynamics-deviation-taylor-map-first-order-eq-stt-eom-taylor-map-1}
...
\subsubsection{\texorpdfstring{4. Substitute
Equation~\ref{eq-stt-taylor-map} into
Equation~\ref{eq-stt-eom-taylor-map}}{4. Substitute Equation~ into Equation~}}\label{substitute-eq-stt-taylor-map-into-eq-stt-eom-taylor-map-1}
...
The first piece is first-order and contributes to the first-order
equation, while the second piece contributes to the second-order
equation.
...
\subsection{Step 5. Balance
coefficients}\label{step-5.-balance-coefficients}
...
are independent variables, their coefficients must be equal. We equate
the coefficients of \(\delta x_{0a}\delta x_{0b}\) from both sides:
