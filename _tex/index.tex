...
\begin{itemize}
\item
  \textbf{Point Mass Dynamics:} For most conjunction scenarios, space
  objects are approximated as point masses, although this assumption may
  be relaxed for large structures like the International Space Station
  (ISS).
\item
  \textbf{State Representation:} In orbital mechanics, the QoIs is
  typically represented by the orbital state vector
  \(\mathbf{x} = [\mathbf{r}, \mathbf{v}]^T \in \mathbb{R}^6\),
  representing position and velocity in an inertial frame (e.g., ECI) or
  a specialized coordinate set like Modified Equinoctial Orbital
  Elements (MEqOE).
\item
  ...
  modeled as Gaussian \(N(\boldsymbol{\mu}_0, \mathbf{P}_0)\). The
  random inputs \(\boldsymbol{\xi}\) may include initial state
  uncertainties, force-modeling errors, and environmental perturbations.
\item
...
\subsection{Classification and Methodology of Uncertainty Propagation
Techniques}\label{classification-and-methodology-of-uncertainty-propagation-techniques}

Managing the \emph{curse of dimensionality} and the inherent
non‑linearity of orbital mechanics requires a robust taxonomy of
methods. These techniques are generally classified by how they represent
the PDF: through point‑based samples, linearized mathematical
transformations, mixture models, polynomial expansions, or hybrid
multi‑fidelity approaches. Each method is assessed based on its ability
to preserve the integrity of the PDF over time, its computational
efficiency, and its suitability for real‑time operational use. The
following sections provide a detailed overview of these methods,
highlighting their theoretical foundations, practical implementations,
...
Gaussian distribution using the square‑right matrix (\(S\)) of the
covariance matrix.
...
\section{Linearized Covariance
(LinCov)}\label{linearized-covariance-lincov}
...
\begin{itemize}
\item
  \textbf{Advantage:} Captures higher-order nonlinear effects for each
  mixand, significantly outperforming single-run linearization.
\item
  \textbf{Limitation:} Computationally expensive for large mixtures, as
  total evaluations scale as \(N_g \times (2n+1)\).
\end{itemize}
...
\begin{itemize}
\item
  \textbf{Methodology:} Uses low-fidelity dynamics (e.g., J2-only) to
  propagate many samples, corrected by a tiny subset of high-fidelity
  evaluations.
\item
...
\begin{itemize}
\item
  \textbf{Methodology:} Kernels are evolved using high-order Taylor
  expansions of the flow.
\item
  GMM-STT Effectively mitigates the divergence of STTs in
  high-uncertainty regions by keeping the individual kernel covariances
  ``small'' and within the radius of convergence.
\end{itemize}

\subsubsection{GMM-DA (Differential
Algebra)}\label{gmm-da-differential-algebra}

Integrates GMMs with high-order Taylor polynomials generated
automatically via DA.
...
\section{Moment Method}\label{moment-method}
...
\begin{itemize}
\item
  \textbf{Taylor Series Expansion:} The core mechanism involves
  expanding the nonlinear system function \(y = f(x)\) or the numerical
  flow map \(\phi(t; x_0)\) into a Taylor series about the mean value of
  the inputs.
\item
  ...
  \item
    ...
    the governing differential equations \(\dot{x} = f(x, t)\) are
    known. It requires a numerical integrator (e.g., RKF78) to define
    the solution flow \(\phi\), which is then expanded.
  \end{itemize}
\item
  \textbf{Multivariate Extension (\(R^n\)):} In a 6-dimensional orbital
  state, the method propagates the mean vector \(\mu\) and the
  covariance matrix \(\Sigma\). For a first-order approximation, the
  covariance propagates as \(\Sigma_{y} = J \Sigma_{x} J^T\), where
...
To handle the ``loss of Gaussianity'' typical in long-term orbital
propagation, the Moment Method is often integrated into a \textbf{GMM
framework}.
...
\begin{itemize}
\item
  \textbf{Sigma Point Paradigm:} While MC requires \(10^5\) to \(10^7\)
  random samples to achieve statistical convergence, the UT utilizes a
  minimal, carefully selected set of weighted \textbf{sigma points} that
  precisely capture the initial mean and covariance of a distribution.
\item
...
The UT approximates an \(n\)-dimensional random variable \(\mathbf{x}\)
using \(2n+1\) sigma points. The mathematical selection and weight
assignment are governed by the following formulations:

\subsubsection{\texorpdfstring{\textbf{Sigma Point
Selection}}{Sigma Point Selection}}\label{sigma-point-selection}
...
\subsubsection{\texorpdfstring{\textbf{Weight
Allocation}}{Weight Allocation}}\label{weight-allocation}
...
Reconstruction}}{Statistical Reconstruction}}\label{statistical-reconstruction}
...
\subsection{Comparative Performance and Hybrid
Integration}\label{sec-ut-performance}
...
\item
  \textbf{Gaussian Mixture Model (GMM) Integration:} To overcome the
  Gaussian limitation, the UT is frequently used to propagate the
  individual kernels of a \textbf{GMM (GMM\_UT)}. The GMM-UT method
  operates on the intuition that while the global distribution may be
  highly non-Gaussian (e.g., a ``banana'' shape), the local regions of
  the probability mass remain nearly Gaussian for a longer duration
  ...
  \item
    ...
    generated using the component's mean \(\mathbf{x}_i\) and covariance
    \(\mathbf{P}_i\).
  \item
    ...
    \(\mathbf{f}(\cdot)\).
  \item
    \textbf{Moment Reconstruction:} The transformed mean and covariance
    for \emph{each} kernel are reconstructed using standard UT weighting
    rules, preserving second-order accuracy.
  \item
    \textbf{Re-Synthesis:} The final non-Gaussian PDF is the re-weighted
    sum of these nonlinearly propagated Gaussian kernels with their
...
In the rigorous hierarchy of uncertainty propagation (UP) methodologies,
\textbf{Polynomial Chaos Expansions (PCE)} represent a powerful class of
\textbf{non-intrusive spectral techniques}. Situated within the broader
category of \textbf{Expansion and Surrogate Methods}, PCE bridges the
gap between local Taylor-series-based approximations (like Linearized
Covariance or State Transition Tensors) and brute-force stochastic
simulations (Monte Carlo) by constructing a global functional
representation of a system's stochastic response.
...
The basis functions are chosen to satisfy the orthogonality condition
relative to the input PDF, \(\rho(\xi)\):
...
In astrodynamics, the non-intrusive approach is favored because it
treats the high-fidelity orbit propagator as a ``black box''.
...
\NormalTok{    F {-}{-}\textgreater{} G[Regression: Least{-}Squares Solution]}
...
\item
  \textbf{Hybrid Meta-Modeling (PC-Kriging):} \textbf{Polynomial
  Chaos-Kriging (PCK)} combines the global trend capture of PCE with the
  localized interpolation accuracy of \textbf{Kriging} (Gaussian Process
  Regression). In this architecture, a sparse PCE represents the global
  system behavior while Kriging interpolates the localized
...
\subsection{Mathematical Foundations of
DA}\label{mathematical-foundations-of-da}

The fundamental principle of DA is to represent any \(k\)-differentiable
function as its Taylor expansion up to a specified order. This is
formalized through the definition of the \textbf{\(nDv\) algebra}, often
denoted as \(kDn\) in literature.
...
\item
  ...
  8,008 coefficients.
\item
  \textbf{The Taylor Map:} The output of a DA-based numerical
  integration is a high-order multidimensional Taylor map,
  \(\mathcal{T}_{x_0}^{(k)}\), which relates initial perturbations
  \(\delta x_0\) to the future state \(x(t)\):
  ...
  \item
    \(\alpha\): A multi-index \((\alpha_1, \dots, \alpha_n)\) where
    \(\alpha_i \in \mathbb{N} \cup \{0\}\).
  \item
    \(c_{\alpha}(t)\): Time-varying polynomial coefficients representing
    partial derivatives.
  \item
...
\item
  ...
  \textbf{Linearised Covariance (LinCov)} method.
\item
...
The STT framework models the flow of a nonlinear dynamical system
\(\dot{\mathbf{x}} = \mathbf{f}(\mathbf{x}, t)\) by expanding the
solution in terms of initial state deviations.

\subsubsection{The High-Order Taylor
Map}\label{the-high-order-taylor-map}

The state deviation \(\delta x_i\) at a future time \(t\) is
approximated as: \[
...
\item
  ...
  the final time.
\item
  \(\delta x_{0, k_p}\): The \(k_p\)-th component of the initial state
  deviation.
\item
  \(P\): The highest order of the Taylor expansion (typically \(P=2\) or
  \(P=3\)).
\item
  \(\phi_{i, k_1 \dots k_p}\): The \(p\)-th order \textbf{State
  Transition Tensor}, representing the \(p\)-th partial derivative of
  the flow with respect to initial conditions.
\end{itemize}

\subsubsection{Differential Equations for Tensor
Propagation}\label{differential-equations-for-tensor-propagation}

The STTs themselves are time-varying and are obtained by numerically
integrating their corresponding ordinary differential equations (ODEs)
alongside the nominal trajectory:
...
\begin{itemize}
\item
  \(\dot{\phi}_{i, a}\): The time derivative of the first-order tensor
  (the State Transition Matrix, STM).
\item
  \(A_{i, k_1 \dots k_p}\): The \(p\)-th partial derivative of the
  dynamics vector field \(\mathbf{f}\) evaluated along the reference
  trajectory:
  \(A_{i, k_1 \dots k_p} = \frac{\partial^p f_i}{\partial x_{k_1} \dots \partial x_{k_p}}\).
\end{itemize}
...
Unlike sampling methods that propagate individual points, STT-based UQ
maps the initial statistical moments directly to the terminal state
...
\begin{itemize}
\item
  \textbf{Directional State Transition Tensors (DSTT):} Reduces
  complexity by aligning the STT terms only with the most sensitive
  directions (eigenvectors of the Cauchy-Green tensor) while ignoring
  stable directions. This can reduce the number of integrated variables
  from \(n^3\) to approximately \(n + n^2 + nm^2\).
\item
...
Multi-Fidelity}\label{method-1-stochastic-collocation-sc-multi-fidelity}
...
Julier, Simon J., and Jeffrey K. Uhlmann. 1997. {``New Extension of the
