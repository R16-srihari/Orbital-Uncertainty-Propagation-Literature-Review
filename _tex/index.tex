...
of the system's equations of motion, which are of the form
...
The above deviation follows the dynamics of the system, which can be
expressed as a differential equation for the deviation:
...
Using similar logic, we can also derive the Taylor expansion for the
equations of motion (EOM) from Equation~\ref{eq-stt-eom} for the
perturbed state, which is essential for propagating the STTs through
time:
...
where the superscript \(^*\) indicates evaluation along the nominal
trajectory \(\mathbf{x}^*(t)\).
...
equations (ODEs) obtained by substituting
Equation~\ref{eq-stt-taylor-map} and
Equation~\ref{eq-stt-taylor-map-time-derivative} into RHS and LHS of
Equation~\ref{eq-stt-eom-taylor-map} each. (Derivation is provided in
Section~\ref{sec-appendix-stt}). For example, the second-order STT(i.e,
\(P=2\)) evolves according to the following ODE:
...
\section{B: Derivation of the differential equation for the second-order
State Transition Tensor (STT) propagation}\label{sec-appendix-stt}

The \textbf{second-order State Transition Tensor (STT) differential
...
\tightlist
\item
  expanding the solution in a Taylor series
  Equation~\ref{eq-stt-taylor-map},
\item
  substituting it into the Taylor expansion of the dynamics
  Equation~\ref{eq-stt-eom-taylor-map},
\item
  differentiating Equation~\ref{eq-stt-taylor-map} with respect to time
  (Equation~\ref{eq-stt-taylor-map-time-derivative}), and
\item
  \textbf{equating coefficients of equal powers of the initial
  deviation} \(\delta x_0\).
\end{enumerate}

Below is the detailed derivation as described in Park and Scheeres
(2006).

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{\texorpdfstring{Step 1. Taylor expansion of the state
(Equation~\ref{eq-stt-taylor-map})}{Step 1. Taylor expansion of the state (Equation~)}}\label{step-1.-taylor-expansion-of-the-state-eq-stt-taylor-map}
...
This is simply Equation~\ref{eq-stt-taylor-map} truncated after second
order.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{\texorpdfstring{Step 2. Time derivative of
Equation~\ref{eq-stt-taylor-map}}{Step 2. Time derivative of Equation~}}\label{step-2.-time-derivative-of-eq-stt-taylor-map}
...
which is Equation~\ref{eq-stt-taylor-map-time-derivative}. The first
term is first-order (hence ignored in the second order derivation), and
the second term is second-order.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Step 3. Expand the
dynamics}\label{step-3.-expand-the-dynamics}

Equation~\ref{eq-stt-eom-taylor-map} gives
...
\subsection{\texorpdfstring{Step 4. Substitute
Equation~\ref{eq-stt-taylor-map} into
Equation~\ref{eq-stt-eom-taylor-map}}{Step 4. Substitute Equation~ into Equation~}}\label{step-4.-substitute-eq-stt-taylor-map-into-eq-stt-eom-taylor-map}
...
\subsubsection{First contribution}\label{first-contribution}
...
\subsubsection{Second contribution}\label{second-contribution}
...
\subsubsection{Collect all second-order
terms}\label{collect-all-second-order-terms}
...
Equation~\ref{eq-stt-taylor-map-time-derivative} contains
