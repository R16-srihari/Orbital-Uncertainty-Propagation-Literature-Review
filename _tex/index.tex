...
\(\mathcal{S}\to\Gamma^d \subseteq \mathbb{R}^d\), which is the support
of the random variables. The choice of \(\Gamma^d\) depends on the
problem; for example, \(\Gamma^d = [0,1]^d\) for \(d\)-dimensional
uniform random variables in the range \([0,1]\).
...
\subsection{Concept Map of Orbital Uncertainty
Propagation}\label{concept-map-of-orbital-uncertainty-propagation}

Section 1.1 of Fossa et al. (2024) provides a detailed concept map on
the topic of \textbf{Orbit Uncertainty propagation}. This taxonomy is
fundamental for understanding how different methodologies balance
computational efficiency and statistical accuracy in space situational
awareness (SSA) applications.

The map is divided into three primary pillars: \textbf{Linear Methods},
\textbf{Nonlinear Methods}, and \textbf{Coordinates Transformation}.

\subsubsection{1. Linear Methods}\label{linear-methods}

These methods assume that a linearized dynamical model can capture the
evolution of neighboring trajectories and that the uncertainty remains
characterized by a multivariate Gaussian distribution.

\begin{itemize}
\item
  \textbf{Linear Covariance (LinCov):} Uses a first-order Taylor
  expansion to compute the time evolution of the covariance matrix.
\item
  \textbf{CADET (Covariance Analysis Describing Function Technique):} A
  statistical linearization method.
\item
  \textbf{Limitations:} These are computationally ``light'' but lose
  accuracy in highly nonlinear regimes, large initial uncertainties, or
  long propagation windows.
\end{itemize}

\subsubsection{2. Nonlinear Methods}\label{nonlinear-methods}

Nonlinear methods can be further divided into three subcategories based
on their fundamental approach to the UP problem.

\paragraph{\texorpdfstring{\textbf{A. Sample-based
Methods}}{A. Sample-based Methods}}\label{a.-sample-based-methods}

These techniques treat the underlying orbital dynamics as a
\textbf{``black-box,''} which simplifies implementation because they do
not require internal modifications to existing code.

\begin{itemize}
\item
  \textbf{Monte Carlo (MC):} The non-parametric benchmark. While
  accurate and able to handle any nonlinearity, it is often too
  computationally expensive for real-time applications.
\item
  \textbf{Unscented Transform (UT) and Conjugate Unscented Transform
  (CUT):} Deterministic sampling methods that approximate the state pdf
  rather than the transformation itself by using ``sigma points''.
\item
  \textbf{Non-intrusive Polynomial Chaos (NIPC):} Constructs a
  functional representation of the uncertainty by fitting spectral
  coefficients to transformed samples.
\item
  \textbf{Stochastic collocation Multi-fidelity (SC-MF)(Jones and
  Weisman 2019):} A hybrid method included in the mind-map that uses
  stochastic collocation to correct low-fidelity samples with a small
  subset of high-fidelity runs.
\end{itemize}

\paragraph{\texorpdfstring{\textbf{B. Dynamics-based
Methods}}{B. Dynamics-based Methods}}\label{b.-dynamics-based-methods}

Unlike sample-based methods, these require complete mathematical
knowledge of the system's governing equations.

\begin{itemize}
\item
  \textbf{Intrusive PC:} Transforms a stochastic differential equation
  into a higher-dimensional deterministic one using Galerkin projection.
\item
  \textbf{State Transition Tensors (STT):} Captures nonlinear warpings
  by numerically integrating variational equations for higher-order
  partial derivatives.
\item
  \textbf{Differential Algebra (DA):} Automates arbitrary-order Taylor
  expansions by replacing standard arithmetic with polynomial algebra.
\item
  \textbf{DA-LOADS Multi-fidelity (Fossà):} The mind-map explicitly
  labels the author's core contribution here---a multifidelity technique
  that leverages \textbf{Low-Order Automatic Domain Splitting (LOADS)}
  to adaptively manage nonlinearities.
\end{itemize}

\paragraph{\texorpdfstring{\textbf{C. Pdf-based
Methods}}{C. Pdf-based Methods}}\label{c.-pdf-based-methods}

These methods aim to provide a functional or analytical representation
of the evolved probability density function.

\begin{itemize}
\item
  \textbf{Fokker-Planck Equation (FPE):} A partial differential equation
  (PDE) that provides a complete statistical description but suffers
  from the ``curse of dimensionality''.
\item
  \textbf{Liouville's Equation:} A quasi-linear PDE used when the
  diffusion term (process noise) can be neglected.
\item
  \textbf{Gaussian Mixture Models (GMM):} Approximates a non-Gaussian
  pdf as a weighted sum of Gaussian kernels, transforming one complex
  problem into several smaller sub-problems.
\end{itemize}

\subsubsection{3. Coordinates
Transformation}\label{coordinates-transformation}

In his thesis, Fossa emphasizes that nonlinearity is not just an
intrinsic system attribute but is \textbf{coupled with the mathematical
representation} chosen. In the context of UP, this fact can be leveraged
to reduce the complexity of the problem by choosing a set of coordinates
that minimizes the nonlinearity of the system at hand. In this case,
linearized or lower-order UP methods could still provide a sufficient
accuracy in the transformed state pdf while significantly improving the
computational performance (Fossa et al. 2024). Uncertainty propagation
methods were applied to the following coordinate sets in literature:

\begin{itemize}
\item
  \textbf{Cartesian:} Popular for simplicity but highly nonlinear due to
  inverse-square gravity.
\item
  \textbf{Orbital Elements:}

  \begin{itemize}
  \item
    \textbf{Classical (COEs):} Intuitive but contain singularities for
    circular or equatorial orbits.
  \item
    \textbf{Equinoctial (EqOEs, MEEs):} Non-singular sets suitable for
    computation and perturbation analysis.
  \item
    \textbf{Alternate Equinoctial (AEqOEs):} Elements that evolve
    linearly in time under unperturbed Keplerian motion, which is highly
    beneficial for UP.
  \item
    \textbf{\(J_2\)-perturbed Equinoctial Elements:} A set of elements
    that account for the \(J_2\) perturbation, which is significant for
    low Earth orbit (LEO) satellites.
  \item
    \textbf{Generalized Equinoctial Elements (GEqOEs):} A more flexible
    set of elements that can be adapted to any perturbing force derived
    from a potential function, including \(J_2\) and higher-order zonal
    harmonics.
  \end{itemize}
\item
  \textbf{Regularized Coordinates:} Redundant variables like the
  \textbf{Dromo} formulation or \textbf{Kustaanheimo-Stiefel (KS)}
  variables used to regularize dynamics.
\end{itemize}

The following sections provide a detailed overview of the most widely
used methods in the literature, highlighting their theoretical
foundations, practical implementations, and performance characteristics.

\includegraphics[width=8.78in,height=7.16in]{index_files/figure-latex/mermaid-figure-1.png}

\textsubscript{Source:
\href{https://r16-srihari.github.io/Orbital-Uncertainty-Propagation-Literature-Review/index.qmd.html}{Article
Notebook}}
...
\subsection{Mathematical Estimators}\label{mathematical-estimators}

Given an initial 6-dimensional orbital state vector
\(\mathbf{x}_0 \in \mathbb{R}^6\) at initial epoch \(t_0\) governed by
the nonlinear ordinary differential equation
\(\dot{\mathbf{x}} = \mathbf{f}(t, \mathbf{x})\), Monte Carlo draws an
ensemble of \(\Omega\) random realizations from the initial joint PDF
\(\pi(\mathbf{x}_0)\):
...
Each individual sample state is independently propagated forward to
terminal epoch \(t\) via the nonlinear flow mapping
\(\boldsymbol{\phi}(t; \mathbf{x}_0^{(k)}, t_0)\):

\[
\mathbf{x}_k(t) = \boldsymbol{\phi}\left(t; \mathbf{x}_0^{(k)}, t_0\right)
\]

The propagated first statistical moment (mean state vector
\(\boldsymbol{\mu}(t)\)) and second central statistical moment (state
covariance matrix \(\mathbf{P}(t)\)) are evaluated via the sample
estimators:
...
\subsection{Properties of the Sampling-Based
Approach}\label{properties-of-the-sampling-based-approach}
...
\subsubsection{Algorithmic Flow of Monte Carlo
Simulation}\label{algorithmic-flow-of-monte-carlo-simulation}

\includegraphics[width=6.1in,height=7.65in]{index_files/figure-latex/mermaid-figure-18.png}
...
\subsection{Variations and Technological
Extensions}\label{variations-and-technological-extensions}

To mitigate the extreme computational burden of the base Monte Carlo
simulation, several variations have been developed that leverage modern
hardware and statistical theory:

\begin{itemize}
\item
  \textbf{Parallel MC (GPGPU):} Since samples are independent, they are
  ideally suited for massively parallel architectures,. Implementation
  on Graphics Processing Units (GPGPUs) can reduce computational time
  linearly with the number of cores, though it introduces bottlenecks in
  memory transfer (PCI Express bus) and requires re-coding dynamics
  without standard libraries like SPICE.
\item
  \textbf{Importance Sampling (IS) and Cross-Entropy (CE):} In
  conjunction analysis, where collision probability (\(P_C\)) is a
  ``rare event'' (often \(10^{-4}\) to \(10^{-6}\)), brute-force methods
  are computationally exhaustive,. IS biases the sampling toward the
  ``failure set'' (the collision region), and the Cross-Entropy method
  iteratively solves for the optimal proposal distribution.
\item
  \textbf{Surrogate-Assisted MC:} Rather than numerical integration for
  every sample, surrogate models---such as high-order Taylor polynomials
  via Differential Algebra (DA) or Polynomial Chaos Expansions
  (PCE)---are used to rapidly evaluate samples. Specific techniques
  include:

  \begin{itemize}
  \tightlist
  \item
    \textbf{Differential Algebra Monte Carlo (DAMC-\(k\)):} Replaces
    pointwise numerical ODE integration with rapid algebraic evaluation
    of a high-order Taylor series expansion using State Transition
    Tensors (STT)
  \item
    \textbf{Polynomial Chaos Expansion Monte Carlo (PCE-MC):} Uses
    orthogonal polynomial expansions to maintain linear computational
    complexity relative to input dimension
  \item
    \textbf{Dual-Number Taylor Polynomial Maps:} Encodes trajectory
    sensitivities to initial state perturbations and time of closest
    approach, achieving computational speedups of \(100\times\) to
    \(300\times\) when embedded in Cross-Entropy Importance Sampling
  \end{itemize}
\item
  \textbf{Multi-Fidelity (MF) Monte Carlo:} Combines low-fidelity (LF)
  propagators (e.g., 2-body Keplerian dynamics) for a large sample set
  \(N_s\) with a small set (\(r_{mf} \approx 6\text{--}8\)) of
  high-fidelity (HF) integrations selected via pivoted Cholesky or
  stochastic collocation. Linear expansion coefficients correct the LF
  sample space. MF achieves HF-level non-Gaussian accuracy while
  accelerating computation time by up to 4 orders of magnitude.
\item
  \textbf{Modified Chebyshev Picard Iteration (MCPI-MC) with MEqOE:}
  Combines Modified Equinoctial Orbital Elements (MEqOE) with Modified
  Chebyshev Picard Iteration (MCPI) to approximate long-arc trajectory
  segments globally rather than step-by-step. Integrating MEEs expands
  the convergence domain, and incorporating local Taylor series gravity
  expansions enables fast parallel Monte Carlo evaluations across
  complex gravity fields.
\item
  \textbf{Design and Analysis of Computer Experiments (DACE) \&
  Space-Filling Sampling:} Replaces purely random pseudo-sampling with
  space-filling designs such as Latin Hypercube Sampling (LHS). LHS
  partitions variable probability distributions into equiprobable
  strata, enabling the construction of computationally inexpensive
  statistical meta-models (Kriging, Random Forests, Radial Basis
  Functions) for sensitivity analysis and outlier detection.
\item
  \textbf{Latin Hypercube Sampling (LHS):} Extends standard Monte Carlo
  by using stratified space-filling designs that guarantee uniform
  coverage of each input dimension, reducing variance and improving
  efficiency for surrogate model training. When combined with machine
  learning surrogates (e.g., Random Forests) or variance-reduction
  techniques like Importance Sampling, LHS can achieve accurate
  uncertainty propagation with far fewer high-fidelity model
  evaluations, making it suitable for sensitivity analysis and outlier
  detection in high-dimensional astrodynamics problems.
\item
  \textbf{MC from Epoch vs.~MC from TCA:} Baseline ``from Epoch''
  propagation uses high-fidelity models from the initial measurement
  time, while ``from TCA'' propagates uncertainty to the Time of Closest
  Approach (TCA) first and then performs a localized MC simulation,
  significantly saving time when it comes to rare-event probability
  (like collision probability \(P_C\)) estimation.
\end{itemize}
...
\item
  ...
  Where \(G(t)\) is the process noise input matrix (that characterize
  the diffusion of a Brownian motion process) and \(Q(t)\) is the
  process noise covariance, both of which are used to account for
  force-model mismodelling perturbation errors. Bos (2025) does not
  include process noise in his LinCov implementation.
\item
...
\subsection{Algorithmic Implementation of
...
\includegraphics[width=2.88in,height=10.56in]{index_files/figure-latex/mermaid-figure-17.png}
...
\item
  ...
  Elements (MEqOE)}, which render the dynamics more nearly linear than
...
\includegraphics[width=3.06in,height=16.15in]{index_files/figure-latex/mermaid-figure-16.png}
...
\includegraphics[width=6.1in,height=16.56in]{index_files/figure-latex/mermaid-figure-15.png}
...
\subsubsection{The PCE Surrogate
Formula}\label{the-pce-surrogate-formula}
...
\item
  ...
  formed as the product of univariate polynomials, selected according to
  the \textbf{Wiener-Askey scheme}.
\item
...
\subsubsection{Expansion Order and the Curse of
Dimensionality}\label{expansion-order-and-the-curse-of-dimensionality}
...
\subsubsection{Orthogonality and Basis
Selection}\label{orthogonality-and-basis-selection}
...
\subsection{Non-Intrusive PCE
Methodology}\label{non-intrusive-pce-methodology}

Non-intrusive methods are favored in astrodynamics because they treat
high-fidelity propagators as ``black boxes,'' requiring no modification
to existing legacy code.

\subsubsection{Methodology for Coefficient
Estimation}\label{methodology-for-coefficient-estimation}

NIPC relies on a design of numerical experiments to estimate
coefficients. Two primary approaches are utilized:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Pseudo-Spectral Projection:} Exploits basis orthogonality by
  projecting model output onto each basis function using numerical
  integration (e.g., Gaussian quadrature or Smolyak sparse grids).
\item
  \textbf{Least-Squares (LS) Regression:} Also known as point
  collocation, this method minimizes the residual between the expansion
  and model evaluations at a set of sampled points (e.g., Latin
  Hypercube Sampling).
\end{enumerate}

\textbf{Flowchart: NIPC Algorithmic Workflow}

\includegraphics[width=2.88in,height=11.31in]{index_files/figure-latex/mermaid-figure-14.png}

\textsubscript{Source:
\href{https://r16-srihari.github.io/Orbital-Uncertainty-Propagation-Literature-Review/index.qmd.html}{Article
Notebook}}

\subsection{Intrusive PCE Methodology}\label{intrusive-pce-methodology}

Intrusive methods, or \textbf{Intrusive Spectral Projection (ISP)},
reformulate the original deterministic model equations into a larger
system of coupled deterministic equations for the PC coefficients.

\subsubsection{Mathematical Framework: The Galerkin
Projection}\label{mathematical-framework-the-galerkin-projection}

The core of the intrusive method is the \textbf{Galerkin projection},
which ensures the approximation error is orthogonal to the functional
space spanned by the basis. For an ODE system
\(\dot{\boldsymbol{x}} = \boldsymbol{f}(\boldsymbol{x}, t, \boldsymbol{\xi})\),
the PCE is substituted directly: \[
\sum_{j=0}^{P} \dot{\boldsymbol{c}}_j \Psi_j = \boldsymbol{f}\left( \sum_{j=0}^{P} \boldsymbol{c}_j \Psi_j, t, \boldsymbol{\xi} \right)
\]

Multiplying by \(\Psi_k\) and taking the expectation
\(\langle \cdot \rangle\) yields a system of \((P+1) \times n\) coupled
deterministic ODEs.

\subsubsection{Procedural Logic for Intrusive
PCE}\label{procedural-logic-for-intrusive-pce}

Unlike NIPC, IPCE requires a complete ``code rewrite'' to implement
polynomial algebra operators.

\textbf{Flowchart: IPCE Algorithmic Workflow}

\includegraphics[width=2.88in,height=8.15in]{index_files/figure-latex/mermaid-figure-13.png}
...
\includegraphics[width=6.1in,height=11.86in]{index_files/figure-latex/mermaid-figure-12.png}
...
\includegraphics[width=5.56in,height=9.83in]{index_files/figure-latex/mermaid-figure-11.png}
...
(that arises from the initial uncertainty of state)
\(\delta \mathbf{x}(t)\) and its derivative
\(\dot{\delta \mathbf{x}(t)}\) at a future time can be expressed as
functions of the initial deviation \(\delta \mathbf{x}_0\) using a
\(P\)-order Taylor series expansion about a nominal trajectory
\(\mathbf{x}^*(t)\).

\subsubsection{Nonlinear State
Deviation}\label{nonlinear-state-deviation}

Equation~\ref{eq-stt-deviation} defines the deviation of a current state
from a nominal trajectory as the difference between two solution flows
of the system's equations of motion.

\begin{equation}\protect\phantomsection\label{eq-stt-deviation}{
...
\item
  \(\mathbf{\phi}(t; \mathbf{x}_0, t_0)\): The \textbf{solution flow} of
  the system's equations of motion, mapping the initial state
  \(\mathbf{x}_0\) at time \(t_0\) to the state at time \(t\). It
  represents the explicit solution of the system's ordinary differential
  equations.

  \begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-important-color!10!white, colframe=quarto-callout-important-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Solution Flow in Orbital dynamics}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

  The solution flow \(\phi(t; \mathbf{x}_0, t_0)\) is not found in
  closed form for most orbital dynamics problems due to the complexity
  and nonlinearity of the governing equations. This is especially true
  for high-fidelity models that include perturbations such as
  atmospheric drag, solar radiation pressure, and third-body effects.
  Instead, the state at time \(t\) is typically obtained through
  numerical integration of the equations of motion starting from the
  initial state \(\mathbf{x}_0\) at time \(t_0\).

  Hence, in orbital dynamics the solution flow is treated as a
  representation of the numerically propagated state. STT leverages the
  existence of such a solution flow to perform Taylor expansions around
  the nominal trajectory, allowing for the analytical propagation of
  uncertainties without requiring a closed-form solution.

  \end{tcolorbox}
\item
  \(\mathbf{x}_0\): The nominal trajectory initial state.
\item
  \(\delta \mathbf{x}_0\): The initial deviation or perturbation from
  the nominal state.
\end{itemize}

The deviation vector also follows the dynamics of the system (of the
form \(\dot{\mathbf{x}} = \mathbf{f}(\mathbf{x}, t)\)), which is
expressed as the following differential equation:
\begin{equation}\protect\phantomsection\label{eq-stt-eom}{
\dot{\delta \mathbf{x}}(t) = \mathbf{f}(\mathbf{x}^\star+\delta \mathbf{x}, t) - \mathbf{f}(\mathbf{x}^\star, t)
}\end{equation}

The superscript \(\star\) indicates that the nominal trajectory
\(\mathbf{x}^\star(t)\) is used as the reference point for evaluating
the dynamics.

\subsubsection{Taylor Expansion of the Solution
Flow}\label{taylor-expansion-of-the-solution-flow}

To arrive at Equation~\ref{eq-stt-Taylor-map}, we perform a multivariate
Taylor series expansion of the perturbed solution flow
\(\phi(t; \mathbf{x}_0 + \delta \mathbf{x}_0, t_0)\) about the nominal
initial conditions \(\mathbf{x}_0\).

\begin{itemize}
\item
  \textbf{Step 1: Scalar Component Expansion} The \(i\)-th component of
  the perturbed state at time \(t\) can be expressed as a function of
  the initial state components: \[
  x_i(t) = \phi_i(t; x_{0,1} + \delta x_{0,1}, \dots, x_{0,n} + \delta x_{0,n}, t_0)
  \]
\item
  \textbf{Step 2: Multivariate Taylor Series Application} Expanding this
  function about the nominal initial state \(x_{0,j}\) yields: \[
  ...
  The derivative terms here can be compactly represented using the
  \textbf{State Transition Tensor} notation, where the \(p\)-th order
  tensor \(\Phi_{i,k_1 \dots k_p}\) encapsulates the \(p\)-th partial
  derivatives of the solution flow with respect to the initial
  conditions.

  \begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{State Transition Tensor Definition}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

  \(\Phi_{i,k_1 \dots k_p}\) is the \(p\)-th order \textbf{State
  ...
  The set \{k\_1, k\_2, \ldots, k\_n\} is a re-labelling of the original
  state components \{1, 2, \ldots, n\} with one-to-one mapping. This
  notation allows the flexibility to choose k\_j such that it reflects a
  specific ordering or a subset of state components of interest for the
  expansion, while still maintaining the generality of the tensor
  representation.

  \end{tcolorbox}
\item
  \textbf{Step 3: Subtraction of the Nominal Flow} Substituting this
  expanded form back into Equation~\ref{eq-stt-deviation}: \[
  ...
  The zeroth-order term \(\phi_i(t; \mathbf{x}_0, t_0)\) (the nominal
  trajectory) cancels out, leaving only the sum of the higher-order
  partial derivatives.
\end{itemize}
...
The final expression relates initial deviations to future deviations
through a sum of tensors:
\begin{equation}\protect\phantomsection\label{eq-stt-Taylor-map}{
...
\item
  ...
  Transition Tensor}, see the note above.
\item
...
Similar to state deviation, we can also derive the Taylor expansion for
dynamics deviation caused by the initial uncertainty from
Equation~\ref{eq-stt-eom} via a similar pathway. This yields the
following expression for the time derivative of the deviation:

\begin{equation}\protect\phantomsection\label{eq-stt-eom-Taylor-map}{
...
We can also find \(\dot{x}_i(t)\) by differentiating
Equation~\ref{eq-stt-Taylor-map} with respect to time \(t\), which
yields:
\begin{equation}\protect\phantomsection\label{eq-stt-Taylor-map-time-derivative}{
\dot{\delta x}_i(t) \approx \sum_{p=1}^P \frac{1}{p!} \dot{\Phi}_{i; k_1 \dots k_p} \delta x_{0,k_1} \dots \delta x_{0,k_p}
}\end{equation}

Since the initial deviations \(\delta x_{0,k_j}\) are arbitrary (chosen
from the initial uncertainty probability distribution), the time
derivative does not affect them. Therefore, the time derivative of the
deviation is expressed in terms of the time derivatives of the STTs.

\subsubsection{Differential Equations for Tensor
Propagation}\label{sec-stt-ode}

These STT's are not static; they evolve over time according to the
system's dynamics. The evolution of the STTs is governed by a set of
coupled ordinary differential equations (ODEs) derived from
Equation~\ref{eq-stt-Taylor-map}, Equation~\ref{eq-stt-eom-Taylor-map}
and Equation~\ref{eq-stt-Taylor-map-time-derivative}. The following
steps outline the derivation of these ODEs:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Substituting Equation~\ref{eq-stt-Taylor-map} into each
  \(\delta x_{l_j}\) in Equation~\ref{eq-stt-eom-Taylor-map}, we obtain
  a new expression for \(\dot{\delta x}_i(t)\) in terms of the initial
  deviations \(\delta x_{0,k_j}\) and the STTs.
\item
  We then equate this new expression with
  Equation~\ref{eq-stt-Taylor-map-time-derivative}, which also expresses
  \(\dot{\delta x}_i(t)\) in terms of the initial deviations and the
  time derivatives of the STTs. The coefficients of the same monomials
  \(\delta x_{0,k_1} \dots \delta x_{0,k_p}\) on both sides of the
  equation must be equal for every set \{\(k_1,\dots, k_p\)\}, leading
  to a set of ODEs for the STTs.
\end{enumerate}

An example derivation for the first and second-order STT is shown in
Section~\ref{sec-appendix-stt}. The resulting ODEs for the STTs are
...
\item
  ...
  \(\mathbf{x}(t)\) and the tensors \(\phi\) up to the truncation order
  \(P\). The value of \(P\) is chosen based on the nonlinearity index
  taken from the propagation of the deviation \(\delta \mathbf{x}(t)\).
\end{enumerate}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-tip-color!10!white, colframe=quarto-callout-tip-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip \ref*{tip-nonlinearity-index}: Local Nonlinearity Index for STT Order Selection}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

\quartocallouttip{tip-nonlinearity-index} 
...
\item
  \(\Delta\bar x_i\) = ith component obtained by direct nonlinear
...
In practice, the nonlinearity index is computed by propagating a set of
initial deviations \(\delta \mathbf{x}_0\) through both the \(m\)-th
order STT and a direct numerical integration of the full nonlinear
dynamics. The relative error between these two predictions is then
evaluated to determine if the current order \(m\) is sufficient to
capture the system's nonlinearity. If \(\eta_m\) exceeds a predefined
threshold, a higher-order STT is computed to improve accuracy.
...
\tightlist
\item
  \textbf{Initial Moment Characterization:} The initial uncertainty is
  characterized by its statistical moments, which are computed from the
  Moment generating function (MGF) of the initial probability
  distribution, which as we said earlier, is typically assumed to be
  Gaussian. The MGF provides a convenient way to compute the moments of
  a known distribution, which are then used in the algebraic mapping.
\end{enumerate}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note \ref*{tip-mgf}: Moment Generating Function (MGF)}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

\quartocallouttip{tip-mgf} 

For a multivariate random vector such that
\(\mathbf{x}\sim\mathcal N(\boldsymbol{\mu},\mathbf{P})\), the MGF is
defined as:
...
The moments of the distribution can be obtained by partially
differentiating the MGF with respect to components of \(\mathbf{t}\) and
evaluating at \(\mathbf{t} = 0\). That is, the \(k\)-th order moment is
given by:
...
\setcounter{enumi}{2}
\item
  \textbf{Statistical Mapping:} Use Equation~\ref{eq-stt-Taylor-map} and
  the linearity of the expectation operator to compute the terminal mean
  ...
  where \(\delta \mu_i\) is the \(i\)-th component of the terminal mean
  vector and \(\mathbf{P}_{ij}\) is the \((i,j)\)-th component of the
  terminal covariance matrix.
\end{enumerate}
...
\item
  ...
  \]
\item
  Covariance of two random variables \(x(t)\) and \(y(t)\) is defined
  as: \[
...
\includegraphics[width=6.1in,height=13.98in]{index_files/figure-latex/mermaid-figure-10.png}
...
\subsection{Contextual Limitations of STTs in Uncertainty
Propagation}\label{contextual-limitations-of-stts-in-uncertainty-propagation}

Despite their accuracy, STTs are bounded by specific constraints within
the larger UP landscape:

\begin{itemize}
\item
  \textbf{Local Validity:} STTs are localized approximations valid only
  within the \textbf{Radius of Convergence} of the Taylor series; if
  uncertainties are too large, the expansion fails to reconstruct the
  dynamics.
\item
  \textbf{Differentiability:} The method requires the underlying
  dynamics to be continuous and differentiable to compute Local Dynamics
  Tensors (LDTs).
\item
  \textbf{Complexity:} Although the algebraic evaluation of the final
  state is fast, the method is described as \textbf{computationally
  intensive} during the initial phase because of the difficulty involved
  in deriving and integrating the high-order tensors for high-fidelity
  models (including \(J_2\), SRP, and drag).
\end{itemize}
...
Complexity is further compounded by the need to compute high-order
derivatives of the dynamics, which can be prohibitive for high-fidelity
models. Variations like \textbf{Directional STTs} and \textbf{Reduced
STTs} have been developed to mitigate these challenges, but they still
require careful consideration of the trade-offs between accuracy and
computational efficiency.

\section{Directional State Transition Tensors
(DSTT)}\label{directional-state-transition-tensors-dstt}

Within the larger context of orbital uncertainty propagation,
\textbf{Directional State Transition Tensors (DSTT)} represent a
specialized dimensionality-reduction framework derived from the
high-order \textbf{State Transition Tensor (STT)} method, developed
primarily by Boone and McMahon (2023). While STTs provide superior
precision over linearized models by capturing localized nonlinear motion
through Taylor series expansions of the solution flow, they are
fundamentally constrained by the ``curse of dimensionality,'' where the
number of variables to be integrated grows exponentially with the
expansion order. DSTTs mitigate this by leveraging the physical
observation that system nonlinearity is often concentrated along a few
``sensitive'' or unstable directions.

\subsection{Theoretical Framework and
Methodology}\label{theoretical-framework-and-methodology}

The DSTT method reformulates the standard STT expansion by performing an
orthogonal transformation of the initial state space, allowing
derivatives to be taken with respect to a rotated basis that isolates
directions of maximum instability. These STTs can be rotated such that
the derivatives are taken with respect to an alternative basis
\(y\in R^n\) with no loss in information as long as \(y\) constitutes an
orthogonal basis that spans \(R^n\). Thesebases are related through the
transformation \(y = Rx\), where \(R\) corresponds to a linear
transformation matrix.

\subsubsection{The Polynomial
Approximation}\label{the-polynomial-approximation}

The state deviation at a future time \(t_f\) is approximated using the
DSTT mapping: \[
...
\item
  \(\varphi_{i, \gamma_1 \dots \gamma_p}\): The \(p\)-th order DSTT
  elements.
\item
  \(\delta y_{\gamma_j 0}\): The initial state deviation projected onto
  the rotated basis \(y = Rx\).
\item
  \(P\): The maximum truncation order of the Taylor series.
\end{itemize}

\subsubsection{Methodology of the
Transformation}\label{methodology-of-the-transformation}

The construction of the DSTT relies on a linear transformation matrix
\(R \in \mathbb{R}^{m \times n}\), where \(m\) is the number of
sensitive directions (typically \(m \ll n\)). Each row of \(R\)
corresponds to a sensitive direction, identified using the eigenvectors
of the \textbf{Cauchy-Green Tensor (CGT)}.

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Cauchy-Green Tensor (CGT)}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

The Cauchy-Green Tensor is a symmetric, positive-definite matrix that
characterizes the local deformation of a flow. It is computed as the
product of the transpose of the State Transition Matrix (STM) and the
STM itself: \[
C(t_f,t_0) = \Phi(t_f,t_0)^T \Phi(t_f,t_0)
\]

where \(t_f\) is the final time and \(t_0\) is the initial time. The
eigenvalues of the CGT indicate the amount of stretching along each
eigenvector direction, with larger eigenvalues corresponding to more
sensitive directions. From Equation~\ref{eq-lincov-propagation}, we
understand the STM (\(\Phi\)) is a linear mapping of the final
perturbation vector \(\delta x(t_f)\) to the initial perturbation vector
\(\delta x(t_0)\). The CGT is a squaring operation that returns the
relationship between the initial and final distance from the nominal
motion.
...
The distance \(||\delta x(t_f)||^2\) reveals the growth or the decay of
the initial vector perturbation. Furthermore, the eigendecomposition of
the CGT allows identification of the principal directions of expansion
for the dynamical flow, including those that are locally associated with
the largest and smallest stretching.

\end{tcolorbox}

The \(p\)-th order DSTT elements are obtained by rotating the full STT
(\(\phi\)) onto this basis: \[
\varphi_{i, \gamma_1 \dots \gamma_p} = \phi_{i, k_1 \dots k_p} R_{\gamma_1, k_1} \dots R_{\gamma_p, k_p}
\]

By choosing the eigenvector \(\xi_1\) associated with the largest
eigenvalue \(\lambda_1\) of the CGT, the DSTT captures the ``direction
of maximum stretch''. This ensures that the terms which contribute most
significantly to the final state deviation are preserved, while stable
directions are ignored.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Advantages over the Standard STT
Method}\label{advantages-over-the-standard-stt-method}

\begin{itemize}
\item
  \textbf{Polynomial Complexity Growth:} A full \(p\)-th order STT
  contains \(n^{p+1}\) elements, resulting in a total integration burden
  of \(\sum_{p=1}^P n^{p+1}\) variables. In contrast, the DSTT reduces
  this to \(n + n^2 + \sum_{p=2}^P n m^p\) variables.
\item
  \textbf{Numerical Efficiency:} For a standard 6-dimensional orbital
  state (\(n=6\)) with one sensitive direction (\(m=1\)) at second order
  (\(P=2\)), the STT requires integrating 258 variables, whereas the
  DSTT requires only 48.
\item
  \textbf{Computational Speedup:} Numerical studies indicate that DSTTs
  can achieve up to \textbf{1000× speed improvements} in nonlinear
  uncertainty propagation compared to full STTs.
\item
  \textbf{Reduced Storage:} Because only the elements along unstable
  directions are stored, the method is significantly more suitable for
  real-time applications on on-board spacecraft computers with limited
  resources.
\item
  \textbf{Analytical Tractability:} DSTTs allow for the derivation of
  simplified, closed-form equations for propagated mean and covariance,
  bypassing the hundreds of terms required by Isserlis' theorem in full
  higher-order covariance propagation.
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Variations and
Enhancements}\label{variations-and-enhancements}

\subsubsection{Time-Varying Directional STT
(TDSTT)}\label{time-varying-directional-stt-tdstt}

A major limitation of the standard DSTT is that it assumes sensitive
directions are constant and is only valid at a predefined final epoch
\(t_f\). The \textbf{TDSTT} {[}Zhou et al. (2024){]} improves this by
simultaneously integrating the nominal orbit, the Taylor expansion, and
the time-varying eigenvalue-eigenvector pairs. This allows for
uncertainty analysis at any point along the trajectory (on a dense time
grid), which is critical for \textbf{conjunction assessment} where the
time of closest approach (TCA) is not known \emph{a priori}.

\paragraph{\texorpdfstring{\textbf{Methodology}}{Methodology}}\label{methodology}

The TDSTT's primary advantage is its ability to perform uncertainty
analysis on a \textbf{dense time grid} without re-integration.

\begin{itemize}
\item
  \textbf{Warm Start Strategy:} At the initial epoch \(t_0\), the CGT is
  an identity matrix with repeated eigenvalues, causing singularity in
  the derivative equations. The methodology employs a ``warm start'' by
  integrating the full STT for a very small step (e.g., \(1/100,000\) of
  the period) to obtain a non-singular CGT and its associated sensitive
  basis.
\item
  \textbf{Augmented ODE System:} The TDSTT methodology integrates the
  nominal state, the STM, the eigenvalues/eigenvectors, and the
  high-order TDSTT terms simultaneously.
\item
  \textbf{Complexity Reduction:} By utilizing the orthogonality of the
  sensitive directions, the complexity of computing the second-order
  TDSTT derivative is reduced from \(O(n^4m^2)\) to \(O(m)\).
\end{itemize}

\paragraph{\texorpdfstring{\textbf{Derivatives of Eigenvalue-Eigenvector
Pairs}}{Derivatives of Eigenvalue-Eigenvector Pairs}}\label{derivatives-of-eigenvalue-eigenvector-pairs}

The integration of TDSTT requires calculating the time evolution of the
sensitive basis. The derivative of the \(k\)-th eigenvalue \(\lambda_k\)
of the CGT is derived as: \[
...
The derivative of the \(k\)-th eigenvector \(\xi_k\) is represented
linearly as: \[
\dot{\xi}_k = \sum_{p \neq k} B_{k,p} \xi_p
\]
...
\tightlist
\item
  \(B_{k,p}\): The coefficients of the eigenvector derivative, where
  \(B_{k,p} = \frac{1}{\lambda_p - \lambda_k} (\dot{\mathbf{C}} \xi_k, \xi_p)\).
\end{itemize}

\paragraph{\texorpdfstring{\textbf{Derivation of the TDSTT
ODEs}}{Derivation of the TDSTT ODEs}}\label{derivation-of-the-tdstt-odes}

The governing ODE for a second-order TDSTT includes rotation terms
(\(B\)) representing the time derivative of the eigenvectors: \[
...
\begin{itemize}
\item
  \(A_{i, \alpha}, A_{i, \alpha \beta}\): Local dynamics tensors
  (Jacobian and Hessian).
\item
  \(B_{p_j, \gamma_j}\): Coefficients derived from eigenvector
  derivatives.
\end{itemize}

\subsubsection{Multi-Directional
Selection}\label{multi-directional-selection}

While many applications use only the primary sensitive direction
(\(m=1\)), some scenarios involve complex dynamics where multiple
directions are unstable. The DSTT framework supports \(R\) matrices
built from the first \(k\) eigenvectors, balancing the tradeoff between
accuracy and computational cost.

\subsection{Algorithm Flowchart for DSTT/TDSTT
Implementation}\label{algorithm-flowchart-for-dstttdstt-implementation}

\includegraphics[width=5.4in,height=15.81in]{index_files/figure-latex/mermaid-figure-9.png}

\textsubscript{Source:
\href{https://r16-srihari.github.io/Orbital-Uncertainty-Propagation-Literature-Review/index.qmd.html}{Article
Notebook}}

\emph{(Ref: Boone and McMahon (2023) , Zhou et al. (2024) )}

\subsection{Summary Table: Contextual
Positioning}\label{summary-table-contextual-positioning}
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Directional STT (DSTT)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Time-Varying DSTT (TDSTT)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\textbf{Expansion Basis} & Cartesian coordinates & Fixed unstable
direction(s) & Dynamic unstable direction(s) \\
\textbf{Complexity (\(n=6, P=2\))} & 258 variables & 48 variables & 55
variables \\
\textbf{Epoch Flexibility} & Valid at all points & Predefined epoch only
& Valid at all points \\
\textbf{Best Use Case} & Low-dimensional systems & Known orbits
(NRHO/Halo) & New targets / Conjunctions \\
\end{longtable}
}

\section{Reduced State Transition Tensors
(RSTT)}\label{reduced-state-transition-tensors-rstt}

The \textbf{Reduced State Transition Tensors (RSTT)} method, primarily
developed by Roa and Park (2021), serves as a strategic
complexity-reduction technique. While standard \textbf{State Transition
Tensors (STTs)} provide superior precision over linear models by
capturing localized nonlinear motion through Taylor series expansions,
they suffer from an exponential growth in computational burden as the
expansion order increases. RSTT addresses this by isolating and
capturing the \textbf{dominant secular terms} responsible for the growth
of relative separation between neighboring trajectories.

\subsection{Theoretical Foundation: Stabilizing the Solution
Flow}\label{theoretical-foundation-stabilizing-the-solution-flow}

The fundamental methodology of RSTT is built on the premise that the
primary instability in orbital motion---the Lyapunov instability---is
relegated to the time domain, specifically the phasing of the spacecraft
along its orbit.

\begin{itemize}
\item
  \textbf{Sundman Transformation:} To isolate secular trends, RSTT
  employs a time transformation (specifically a Sundman transformation
  of order \(\beta=2\)) to switch the independent variable from physical
  time \(t\) to the true anomaly increment \(\theta\):
  ...
  \tightlist
  \item
    \(\theta\): The true anomaly increment from the initial epoch
    \(t_0\).
  \item
  ...
  \end{itemize}
\item
  \textbf{Decomposition of the Integral Flow:} By transcribing the
  solution into the \(\theta\) domain, the state vector at a future time
  can be expressed as a stabilized solution \(G(\theta; \mathbf{x}_0)\)
  corrected by a time-phasing term \(\Theta(t; \mathbf{x}_0)\): \[
  \mathbf{x}(t) = \mathbf{F}(t; \mathbf{x}_0) = \mathbf{G}(\Theta(t; \mathbf{x}_0); \mathbf{x}_0)
  \]

  This decomposition allows the partial derivatives of the flow (the
  tensors) to be split into a stabilized component and a secular
  correction component.
\end{itemize}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Methodology for RSTT
Approximation}\label{methodology-for-rstt-approximation}

The RSTT methodology reduces complexity by retaining only the
highest-order secular terms (those growing like \(t^p\) for a \(p\)-th
order tensor) while neglecting periodic and lower-order secular
components.

\subsubsection{1. First-Order Correction (The Secular
STM)}\label{first-order-correction-the-secular-stm}

The standard State Transition Matrix (STM) \(\Phi\) can be decomposed
into a stabilized derivative and a first-order secular correction: \[
...
\begin{itemize}
\item
  \(\Gamma_j\): The partial derivative of the time transformation
  \(\Theta\) with respect to initial conditions
  \(\partial \Theta / \partial x_{0,j}\).
\item
  \(\partial G_i / \partial \theta\): The tangent vector to the orbit
  (related to velocity).
\end{itemize}

\subsubsection{2. The Second-Order RSTT
Approximation}\label{the-second-order-rstt-approximation}

The core ``reduction'' occurs at the second order. Instead of
integrating 216 individual elements for an \(n=6\) state, the
second-order RSTT is approximated as:
...
\item
  \(\Gamma_j, \Gamma_k\): Sensitivities of the true anomaly to initial
  state variations.
\item
  \(\partial^2 F_i / \partial t^2\): The time derivative of the dynamics
  (the acceleration/force function).
\end{itemize}

\subsubsection{3. Generalization to Higher
Orders}\label{generalization-to-higher-orders}

Higher-order STTs (\(p \ge 3\)) are similarly reduced to a product of
\(\Gamma\) vectors and higher-order time derivatives of the flow,
growing like \(t^p\):
\[\Phi_{i;k_1 \dots k_p} \approx \frac{1}{\gamma^p} \Gamma_{k_1} \dots \Gamma_{k_p} \frac{\partial^p F_i}{\partial t^p}\]

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Algorithm for RSTT-Based Uncertainty
Propagation}\label{algorithm-for-rstt-based-uncertainty-propagation}

The following flowchart illustrates the implementation of RSTT for
nonlinear covariance propagation.

\includegraphics[width=2.88in,height=11.06in]{index_files/figure-latex/mermaid-figure-8.png}
...
\subsection{Advantages and Contextual
Variations}\label{advantages-and-contextual-variations}

RSTT exists as a middle ground between the accuracy of full STTs and the
efficiency of directional methods.
...
\item
  ...
  \tightlist
  \item
    \textbf{Variable Reduction:} For a second-order expansion, RSTT
    requires integrating only \textbf{6 additional differential
    equations} (the elements of \(\Gamma\)) in addition to the state and
    STM. This is a drastic reduction from the 216 elements required for
    a full second-order STT.
  \item
    \textbf{Speedup:} Numerical tests in LEO scenarios indicate that
    RSTT reduces the computational time required to compute the STT by
    approximately \textbf{37\%} while maintaining comparable long-term
    accuracy.
  \end{itemize}
\item
  \textbf{Accuracy Convergence:} While full STTs are more accurate in
  the short term (due to periodic and linear secular terms), the RSTT
  approximation converges to the exact STT solution as time advances and
  the \(t^p\) secular terms become dominant.
\item
  \textbf{Comparison with Other Methods:}

  \begin{itemize}
  \item
    \textbf{RSTT vs.~Full STT:} RSTT avoids the ``curse of
    dimensionality'' and the manual derivation of complex
    Hessian/third-order Local Dynamics Tensors.
  \item
    \textbf{RSTT vs.~DSTT (Directional STT):} While DSTT reduces
    dimensionality by aligning with ``sensitive directions'' of the
    Cauchy-Green Tensor, RSTT reduces complexity by isolating the
    ``secular time drift'' common to all orbital perturbations.
  \item
    \textbf{RSTT vs.~UT (Unscented Transform):} RSTT-based filters (like
    the ASEKF) can be several times faster than the UKF because they
    replace sigma-point sampling with a single algebraic correction.
  \end{itemize}
\end{itemize}

\subsection{Variations on the RSTT
Method}\label{variations-on-the-rstt-method}

\begin{itemize}
\item
  \textbf{ASEKF (Approximate Second-Order EKF):} A high-order Kalman
  filter that utilizes the RSTT approximation for the prediction step.
  It yields results comparable to the UKF but with roughly \textbf{50\%
  faster} computation than an EKF using exact STTs.
\item
  \textbf{Analytical RSTT for Two-Body Problems:} In purely Keplerian
  regimes, the \(\Gamma\) vector and time derivatives can be expressed
  in closed form, allowing for instantaneous non-linear UP without any
  numerical integration.
\item
  \textbf{Vectorized DT (Distribution Transport):} A related evolution
  that uses the same Taylor foundation but transforms the tensor
  structure into a fixed \textbf{vector structure}, enabling parallel
  computing speedups of 14x to 64x.
\end{itemize}

\section{Event Transition Tensors
(ETT)}\label{event-transition-tensors-ett}

In the field of high-order uncertainty propagation (UP), \textbf{Event
Transition Tensors (ETTs)} represent a significant extension of the
\textbf{State Transition Tensor (STT)} framework. While standard STTs
are parametric tools used to map state deviations and statistical
moments to a predefined future time, ETTs provide a rigorous
mathematical description of system behavior on \textbf{differentiable
event manifolds}.

This methodology is critical for aerospace applications where the epoch
of interest is defined by a physical condition (e.g., atmospheric entry,
planetary landing, or a surface of section crossing) rather than a fixed
timestamp.

\subsubsection{1. Theoretical Foundation: From Solution Flows to Event
Maps}\label{theoretical-foundation-from-solution-flows-to-event-maps}

To understand ETTs, one must distinguish between the standard solution
flow of an ordinary differential equation (ODE) and the specialized
event map.

\begin{itemize}
\item
  \textbf{The Solution Flow (\(\Phi\)):} In the STT framework, the flow
  represents all possible trajectories generated by varying initial
  conditions \(\mathbf{x}_0\) and parameters \(\theta\) over a fixed
  time \(t\). The coefficients of the Taylor expansion of this flow are
  the STTs.
\item
  \textbf{The Event Manifold (\(e\)):} An event is implicitly defined by
  the condition \(e(\mathbf{x}, \theta, t) = 0\), representing a target
  surface or a specific state condition.
\item
  \textbf{The Event Map (\(\mathcal{F}\)):} The event map is defined as
  the flow of the dynamical system evaluated at the \textbf{trigger time
  \(t^*\)}, where \(t^*\) is the time at which the trajectory intersects
  the manifold.
\item
  \textbf{The Definition of ETTs:} ETTs are the coefficients of the
  multivariate Taylor expansion of the event map \(\mathcal{F}\).
  Mathematically, if the order of the expansion is \(k\), the state on
  the event manifold \(\mathbf{x}_e\) is approximated as: \[
  ...
  \begin{itemize}
  \item
    \(\delta x_{ie}\): The \(i\)-th component of the state deviation on
    the event manifold.
  \item
    \(\alpha\): An \(n\)-tuple multi-index representing the partial
    derivative order.
  \item
    \(\delta \mathbf{z}\): The vector combining initial state
    perturbations \(\delta \mathbf{x}_0\) and parameter deviations
    \(\delta \theta\).
  \end{itemize}
\end{itemize}

\subsubsection{2. Methodology: Computation via Augmented Systems and
Inversion}\label{methodology-computation-via-augmented-systems-and-inversion}

The methodology for computing ETTs involves transforming a variable-time
event problem into a fixed-time integration problem through the use of
differential algebra or variational equations.

\begin{itemize}
\item
  \textbf{Step 1: System Augmentation} The system of ODEs
  \(\dot{\mathbf{x}} = \mathbf{f}(\mathbf{x}, \theta, t)\) is augmented
  with the derivative of the event function
  \(\epsilon = e(\mathbf{x}, \theta, t)\). To recast the problem for a
  fixed integration interval, a normalized time variable \(\tau = t/T\)
  is introduced, where \(T\) is the unknown event trigger time. \[
  ...
  \]
\item
  \textbf{Step 2: Integration of Variational Equations} By integrating
  the \(k\)-th order variational equations of this augmented system up
  to \(\tau = 1\), one obtains a polynomial representation of the final
  state \(\delta \mathbf{x}_f\) and the event variation
  \(\delta \epsilon_f\) as functions of initial perturbations and the
  time correction \(\delta T\).
\item
  \textbf{Step 3: Map Inversion} Utilizing the \textbf{implicit function
  theorem}, the polynomial system is partially inverted. By forcing the
  final event variation \(\delta \epsilon_f\) to zero, the variation in
  the trigger time \(\delta T\) is expressed as a function of the
  initial perturbations. Substituting this result back into the state
  polynomial yields the ETTs, which define the system state constrained
  strictly to the event manifold.
\end{itemize}

\subsubsection{3. Algorithm Flowchart for ETT
Propagation}\label{algorithm-flowchart-for-ett-propagation}

\includegraphics[width=2.88in,height=13.73in]{index_files/figure-latex/mermaid-figure-7.png}

\textsubscript{Source:
\href{https://r16-srihari.github.io/Orbital-Uncertainty-Propagation-Literature-Review/index.qmd.html}{Article
Notebook}}

\subsubsection{4. Variations and Comparative
Context}\label{variations-and-comparative-context}

ETTs exist in a continuum with STTs and other high-order methods, with
several variations depending on the application:

\begin{itemize}
\item
  \textbf{ETTs vs.~STTs:} If the event manifold is defined as a reaching
  a predefined constant time \(T\), the ETTs collapse into standard
  STTs.
\item
  \textbf{Poincaré Maps:} In periodic systems, ETTs are used to
  construct discrete Poincaré maps {[}Poincaré return maps{]}. This is
  done by defining the surface of section (e.g.,
  \(x_{jf} - \bar{x}_{j0} = 0\)) and inverting the map to find the
  period correction \(\delta t^*\) required for the return.
\item
  \textbf{NeuralODEs expansion:} A novel variation involves using ETTs
  to analyze \textbf{Neural Ordinary Differential Equations}. Because
  NeuralODEs often function as ``black-box'' systems, ETTs provide a way
  to certify their reliability by expressing their complex learned
  dynamics through explicit, high-order mathematical structures on event
  boundaries.
\item
  \textbf{Non-Gaussian Handling:} ETTs can be combined with
  \textbf{Moment Generating Functions (MGFs)} to propagate non-Gaussian
  uncertainties (e.g., uniform distributions for spacecraft mass or
  position) to an event manifold without requiring Monte Carlo sampling.
\end{itemize}

\subsubsection{5. Implementation
Advantages}\label{implementation-advantages}

\begin{itemize}
\item
  \textbf{Computational Efficiency:} Mapping statistical moments
  directly to an event manifold eliminates the need for ``stopping''
  numerical integrators for each sample in a Monte Carlo run.
\item
  \textbf{Rigorous Certification:} In safety-critical missions, such as
  landing a quadcopter through a gate or a spacecraft on an asteroid,
  ETTs allow for the analytical verification of performance requirements
  (e.g., relative velocity \textless{} 0.5 m/s in 99\% of cases) by
  bounding all possible configurations on the manifold.
\item
  \textbf{Automation:} Modern pipelines, such as the open-source
  \textbf{heyoka} project, have automated the symbolic derivation and
  integration of these tensors, making the manual derivation of
  high-order event sensitivities unnecessary.
\end{itemize}
...
\includegraphics[width=6.1in,height=12.47in]{index_files/figure-latex/mermaid-figure-6.png}
...
\paragraph{\texorpdfstring{\textbf{The Radius of Convergence Problem and
GMM
mitigation}}{The Radius of Convergence Problem and GMM mitigation}}\label{the-radius-of-convergence-problem-and-gmm-mitigation}

STTs approximate the solution flow of a dynamical system using a
high-order Taylor series expansion about a nominal trajectory. This
expansion is localized and only valid within a finite neighborhood known
as the \textbf{radius of convergence} {[}Park and Scheeres (2006){]}
{[}Acciarini et al. (2025){]}.

\begin{itemize}
\tightlist
\item
  \textbf{Estimating Validity:} To compare how well the Taylor
  expansions can reconstruct the actual nonlinear dynamics, we can, as
  an initial analysis of the convergence radius, use results from basic
  calculus that study the convergence of power series{[}Acciarini et al.
  (2025){]}. Tools like the \textbf{Cauchy--Hadamard theorem} are used
  as a proxy to estimate a convergence radius and check if the initial
  uncertainties are within the valid region.
\end{itemize}

\marginnote{\begin{footnotesize}

The \textbf{Cauchy--Hadamard} convergence radius of a Taylor expansion
series
\(\delta x_f^i \approx \mathcal{P}_i^k(\delta z) = \sum_{|\alpha|=1}^{k} \frac{1}{\alpha!} \left( \partial^\alpha x_f^i \right)\bigg|_{(\bar{x}_0, \bar{q})} \, \delta z^\alpha\)
is given by: \[
R_c = \frac{1}{\lim_{k \to \infty} sup_{|\alpha|=k} |a_{\alpha}\sqrt{\alpha!/k!}|^{1/k}}
\]

Where \(a_{\alpha}\) is the \(\alpha\)-coefficient of the Taylor series,
with \(\alpha\) being the n-tuple already introduced in the expansion.
...
\tightlist
\item
  \textbf{Failure Mode:} If the initial state deviations are too large
  or if the tails of the probability density function (PDF) extend
  beyond this \(R_c\), the STT expansion fails to accurately reconstruct
  the nonlinear dynamics, leading to significant errors.
\end{itemize}
...
\begin{itemize}
\item
  \textbf{Decomposition into Sub-Gaussians:} The initial uncertainty
  distribution is split into a finite sum of smaller, weighted Gaussian
  components.
\item
  \textbf{Ensuring Local Convergence:} By making each individual
  Gaussian component ``small enough,'' the GMM ensures that each
  component's domain is fully contained within the STT's \textbf{linear
  region of convergence} or its high-order radius of convergence.
\item
  \textbf{Maintaining Phasing Accuracy:} In long-term propagation, the
  uncertainty stretches along the trajectory (the direction of maximum
  nonlinearity). Splitting the distribution---especially in coordinate
  spaces like \textbf{equinoctial elements}---allows these components to
  stay within accurate bounds even as global non-Gaussianity increases.
\item
  ...
  \item
    ...
    \(i\)-th Gaussian component; \(\Phi^k_{j_1...j_p}\) is the \(p\)-th
    order State Transition Tensor.
  \end{itemize}
\end{itemize}

\paragraph{\texorpdfstring{\textbf{Benefits of Tandem GMM-STT
Propagation}}{Benefits of Tandem GMM-STT Propagation}}\label{benefits-of-tandem-gmm-stt-propagation}

\begin{itemize}
\item
  \textbf{Reduced Expansion Order:} Because individual GMM components
  experience less severe ``localized'' nonlinearity than the original
  distribution, they can be accurately mapped using \textbf{lower-order
  STTs} (e.g., second-order). This provides high accuracy while avoiding
  the computational complexity of very high-order tensors.
\item
  \textbf{Mitigation of Distribution Tails:} Using GMMs effectively
  mitigates issues arising from distribution tails that would otherwise
  extend beyond the \(R_c\). This ensures the PDF has a compact domain
  fully contained within the valid expanded flow.
\item
  \textbf{Adaptive Efficiency:} Techniques like \textbf{Automatic Domain
  Splitting (ADS)} or \textbf{AEGIS} can detect when a component is
  nearing the edge of its valid convergence region (using entropy or
  truncation error thresholds) and trigger an online split, keeping the
  propagation statistically consistent.
\end{itemize}
...
\includegraphics[width=2.88in,height=14.56in]{index_files/figure-latex/mermaid-figure-5.png}
...
Developed by Fossà et al.(Fossa et al. 2024), this method combines the
local accuracy of Taylor expansions with the global flexibility of
...
\includegraphics[width=6.1in,height=13.37in]{index_files/figure-latex/mermaid-figure-4.png}
...
\item
  ...
  the \textbf{diffusion} term, which accounts for the broadening of the
  PDF due to stochastic forcing.
\item
...
\includegraphics[width=3.73in,height=15.49in]{index_files/figure-latex/mermaid-figure-3.png}
...
\section{A: Pivoted Cholesky Decomposition for Multi-Fidelity Node
Selection}\label{sec-appendix-pivoted-cholesky}
...
\includegraphics[width=6.87in,height=16.14in]{index_files/figure-latex/mermaid-figure-2.png}

\textsubscript{Source:
\href{https://r16-srihari.github.io/Orbital-Uncertainty-Propagation-Literature-Review/index.qmd.html}{Article
Notebook}}

\section{B: Derivation of the differential equation for first and
second-order State Transition Tensor (STT)
propagation}\label{sec-appendix-stt}

\subsection{First-order STT}\label{first-order-stt}

To derive the differential equation for the first-order STT
\(\Phi_{i;a}\), we need the equations Equation~\ref{eq-stt-Taylor-map},
Equation~\ref{eq-stt-eom-Taylor-map} and
Equation~\ref{eq-stt-Taylor-map-time-derivative} in first order and
follow the steps outlined in Section~\ref{sec-stt-ode} to match the
coefficients of \(\delta x_{0a}\) on both sides of the resulting
equation.

\subsubsection{\texorpdfstring{1. First order Taylor expansion of the
state deviation
(Equation~\ref{eq-stt-Taylor-map})}{1. First order Taylor expansion of the state deviation (Equation~)}}\label{first-order-taylor-expansion-of-the-state-deviation-eq-stt-taylor-map}

\begin{equation}\protect\phantomsection\label{eq-stt-Taylor-map-first-order}{
\delta x_i(t) = \sum_a\Phi_{i;a},\delta x_{0a}
}\end{equation}

This is Equation~\ref{eq-stt-Taylor-map} in first order.

\subsubsection{2. Time derivative of state deviation Taylor
map}\label{time-derivative-of-state-deviation-taylor-map}

Since only the STTs depend on time,

\begin{equation}\protect\phantomsection\label{eq-stt-Taylor-map-time-derivative-first-order}{
\delta\dot x_i = \sum_a \dot\Phi_{i;a}\delta x_{0a}
}\end{equation}

which is Equation~\ref{eq-stt-Taylor-map-time-derivative} in first
order.

\subsubsection{\texorpdfstring{3. Dynamics deviation Taylor map first
order
(Equation~\ref{eq-stt-eom-Taylor-map})}{3. Dynamics deviation Taylor map first order (Equation~)}}\label{dynamics-deviation-taylor-map-first-order-eq-stt-eom-taylor-map}

Equation~\ref{eq-stt-eom-Taylor-map} in first order gives

\begin{equation}\protect\phantomsection\label{eq-stt-eom-Taylor-map-first-order}{
...
\subsubsection{4. Substitution of State deviation Taylor map into
dynamics Taylor
map}\label{substitution-of-state-deviation-taylor-map-into-dynamics-taylor-map}
...
using Equation~\ref{eq-stt-Taylor-map-first-order}. Then the
Equation~\ref{eq-stt-eom-Taylor-map-first-order} becomes

\[
\delta\dot x_i = \sum_{\gamma} \sum_a f^\star_{i;\gamma}\Phi_{\gamma;a}\delta x_{0a}
\]

\subsubsection{5. Balancing coefficients}\label{balancing-coefficients}

Now we equate both
Equation~\ref{eq-stt-Taylor-map-time-derivative-first-order} and the
modified Equation~\ref{eq-stt-eom-Taylor-map-first-order}. Since
\(\delta x_{0a}\) are independent variables, their coefficients must be
equal. Removing the common factor, we obtain the first-order STT
differential equation:

\begin{equation}\protect\phantomsection\label{eq-first-order-stt-differential-equation}{
\boxed{
\dot\Phi_{i;a} = \sum_{\gamma} f^\star_{i;\gamma}\Phi_{\gamma;a}
}
}\end{equation}

\subsection{Second-order STT}\label{second-order-stt}

To derive the differential equation for the second-order STT
\(\Phi_{i;ab}\), we need the equations Equation~\ref{eq-stt-Taylor-map},
Equation~\ref{eq-stt-eom-Taylor-map} and
Equation~\ref{eq-stt-Taylor-map-time-derivative} in second order and
follow the steps outlined in Section~\ref{sec-stt-ode} to match the
coefficients of \(\delta x_{0a}\delta x_{0b}\) on both sides of the
resulting equation.

\subsubsection{\texorpdfstring{1. Second order Taylor expansion of the
state
deviation(Equation~\ref{eq-stt-Taylor-map})}{1. Second order Taylor expansion of the state deviation(Equation~)}}\label{second-order-taylor-expansion-of-the-state-deviationeq-stt-taylor-map}

\begin{equation}\protect\phantomsection\label{eq-stt-Taylor-map-second-order}{
\delta x_i(t) = \sum_a\Phi_{i;a},\delta x_{0a} + \frac12\sum_{a,b}\Phi_{i;ab},\delta x_{0a}\delta x_{0b}
}\end{equation}

This is Equation~\ref{eq-stt-Taylor-map} truncated after second order.

\subsubsection{2. Time derivative of state deviation Taylor
map}\label{time-derivative-of-state-deviation-taylor-map-1}

Since only the STTs depend on time,

\begin{equation}\protect\phantomsection\label{eq-stt-Taylor-map-time-derivative-second-order}{
\delta\dot x_i = \sum_a \dot\Phi_{i;a}\delta x_{0a} + \frac12\sum_{a,b}\dot\Phi_{i;ab}\delta x_{0a}\delta x_{0b}
}\end{equation}

which is Equation~\ref{eq-stt-Taylor-map-time-derivative} in second
order.

\subsubsection{\texorpdfstring{3. Dynamics deviation Taylor map second
order
(Equation~\ref{eq-stt-eom-Taylor-map})}{3. Dynamics deviation Taylor map second order (Equation~)}}\label{dynamics-deviation-taylor-map-second-order-eq-stt-eom-taylor-map}

Equation~\ref{eq-stt-eom-Taylor-map} truncated to second order gives

\begin{equation}\protect\phantomsection\label{eq-stt-eom-Taylor-map-second-order}{
...
\subsubsection{4. Substitution of state deviation Taylor expansions to
dynamics Taylor
map}\label{substitution-of-state-deviation-taylor-expansions-to-dynamics-taylor-map}

Substituting Equation~\ref{eq-stt-Taylor-map-second-order} into the
first \(\delta x_{\gamma}\) of
Equation~\ref{eq-stt-eom-Taylor-map-second-order} gives
...
\paragraph{\texorpdfstring{\textbf{First
contribution}}{First contribution}}\label{first-contribution}
...
The first piece is first-order and is part of
Equation~\ref{eq-first-order-stt-differential-equation}, while the
second piece contributes to the second-order equation.

\paragraph{\texorpdfstring{\textbf{Second
contribution}}{Second contribution}}\label{second-contribution}
...
For deriving the second-order equation, only first-order parts of each
state are needed because
...
and does not become part of the second-order equation. Therefore, we
only need to substitute the first-order Taylor expansion of the states
into the second contribution, which are
...
This is the entire second-order contribution from the nonlinear
dynamics.

\paragraph{\texorpdfstring{\textbf{Collect all second-order
terms}}{Collect all second-order terms}}\label{collect-all-second-order-terms}
...
Equation~\ref{eq-stt-Taylor-map-time-derivative-second-order} contains
...
\subsubsection{5. Balancing
coefficients}\label{balancing-coefficients-1}

We balance the coefficients of the second order terms
\(\delta x_{0a}\delta x_{0b}\) on both sides of the equation, which
gives \[
...
Removing the common factor (1/2), we get the final equation for the
second-order STT differential equation:

\begin{equation}\protect\phantomsection\label{eq-second-order-stt-differential-equation}{
\boxed{
\dot\Phi_{i;ab}=\sum_{\gamma, \eta}\left[f^\star_{i;\gamma}\Phi_{\gamma;ab}+f^\star_{i;\gamma\eta}\Phi_{\gamma;a}\Phi_{\eta;b}\right]}
}\end{equation}

First order STT's \(\Phi_{i;a}\) and \(\Phi_{i;b}\) are obtained by
solving the first-order STT differential equation, which is a
prerequisite for solving the second-order equation.
...
\bibitem[\citeproctext]{ref-boone-mcmahon:10.2514ux2f1.G006910}
Boone, Spencer, and Jay McMahon. 2023. {``Directional State Transition
Tensors for Capturing Dominant Nonlinear Effects in Orbital Dynamics.''}
\emph{Journal of Guidance, Control, and Dynamics} 46 (3): 431--42.
...
\bibitem[\citeproctext]{ref-fossa2024multifidelity}
Fossa, Alberto, Daniel Alazard, Emmanuel Delande, and Roberto Armellin.
2024. {``Multifidelity Orbit Uncertainty Propagation in the Presence of
Stochastic Accelerations.''} PhD thesis, Ph. D. Thesis, Inst.
Sup{é}rieur de l'A{é}ronautique et de l'Espace, Toulouse~\ldots.
\url{https://depozit.isae.fr/theses/2024/2024_Fossa_Alberto.pdf}.
...
Inst. Sup{é}rieur de l'A{é}ronautique et de l'Espace, Toulouse.

\bibitem[\citeproctext]{ref-fujimoto-scheeres:10.2514ux2f1.G000795}
Fujimoto, K., and D. J. Scheeres. 2015. {``Tractable Expressions for
Nonlinearly Propagated Uncertainties.''} \emph{Journal of Guidance,
Control, and Dynamics} 38 (6): 1146--51.
\url{https://doi.org/10.2514/1.G000795}.

\bibitem[\citeproctext]{ref-Izzo_Acciarini:10.1126ux2fsciadv.ady1348}
Izzo, Dario, Sebastien Origer, Giacomo Acciarini, and Francesco Biscani.
2025. {``High-Order Expansion of Neural Ordinary Differential Equation
Flows.''} \emph{Science Advances} 11 (51): eady1348.
\url{https://doi.org/10.1126/sciadv.ady1348}.
...
\bibitem[\citeproctext]{ref-zhou2024timevaryingdirectionalstatetransition}
Zhou, Xingyu, Roberto Armellin, Dong Qiao, and Xiangyu Li. 2024.
\emph{Time-Varying Directional State Transition Tensor for Orbit
Uncertainty Propagation}. \url{https://arxiv.org/abs/2412.07060}.
