...
\item
  ...
  the initial PDF. Since STT methods propagates the deviation
  \(\delta \mathbf{x}(t)\), the initial moments are computed for the
  zero-mean deviation \(\delta \mathbf{x}_0\) rather than the absolute
  state \(\mathbf{x}_0\). For a Gaussian initial distribution, the
  moments can be computed analytically using \textbf{Isserlis' Theorem}
  (or Wick's formula).
\end{enumerate}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Isserlis' Theorem for Gaussian Moments}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

For a \textbf{zero-mean} multivariate normal random vector
\(\mathbf{x}\), \textbf{Isserlis' Theorem} provides a specific formula
for the expectation of the product of its components.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Odd Orders:} If the total number of components \(k\) is odd,
  the \(k\)th-order central moment is \textbf{zero}.
\item
  \textbf{Even Orders:} If \(k\) is even, the expectation is equal to
  the sum of the products of the covariances of all possible unordered
  pairings of the components: \[
  E[x_{k_1} x_{k_2} \dots x_{k_n}] = \sum_{\text{all pairings}}\prod_{\text{pairs }(i,j)}P_{ij}
  \]

  Where \(P_{ij}\) is the covariance between components \(x_i\) and
  \(x_j\).
\end{enumerate}

For example, the \textbf{fourth-order moment} of a zero-mean Gaussian
...
\emph{(Source:Acciarini et al. (2025))}
