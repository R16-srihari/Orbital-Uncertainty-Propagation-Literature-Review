...
| **Intrusive PCE**                        | High efficiency once governing equations are reformulated              | Requires reformulation of the governing equations                      | Structural rigidity and implementation complexity of stochastic Galerkin system                | The Galerkin system must be derived and solved directly, which increases implementation effort and makes the method less flexible for complex force models.                              | Need access to dynamics source code to perform the Galerkin
projection of the governing dynamics onto a subspace to transform a stochastic ordinary differential equation (SODE) in low-dimension into an equivalent deterministic ordinary differential equation (ODE) in higher dimension | TBD | TBD |
